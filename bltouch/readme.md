...
После прошивки сначала нужно сделать initialize eeprom из меню принтера.
...
* Выполнить команду [G28](https://marlinfw.org/docs/gcode/G028.html) или Auto Home из меню
* Запустить измерение высот командой [G29](https://marlinfw.org/docs/gcode/G029.html)
* Сохранить полученную карту высот командой [M500](https://marlinfw.org/docs/gcode/M500.html) или Store settings из меню
...
* Во время печати юбки из меню Tune-> [BabyStepping](https://marlinfw.org/docs/gcode/M290.html) изменяя высоту стола добиться правильной укладки линий. 
* Когда уровень стола будет устраивать, запомнить итоговое значение BabyStepping и остановить печать.
* Установить значение BabyStepping в качестве Z PROBE OFFSET
...
Если делать калибровку перед каждой печатью, команду G29 нужно добавить в стартовый код. Для того, чтобы делать калибровку с нагретым столом, но без нагрева экструдера, в стартовый код нужно добавить в начало:

```
G28 ;парковка, чтобы обнулить координаты
G29 ;Запустить калибровку
