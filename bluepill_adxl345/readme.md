...
В этом описании я предполагаю, что целевой аудитории нет опыта программирования stm32 и работы с stlink. Поэтому для прошивки платы будет использоваться UART, а все операции сделаны непосредственно с хоста с клиппером (в данном случае Orange Pi 3 LTS).
...
* "cd ~/klipper" - перейдет в домашний каталог текущего пользователя ( "~" будет заменена на /home/username) 
